...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addSubcommand(subcommand => subcommand.setName('casos')
        .setDescription('Muestra los casos de covid19 en Argentina.'))
        .addSubcommand(subcommand => subcommand.setName('global')
        .setDescription('Muestra los casos de covid19  actuales en el mundo'))
        .addSubcommand(subcommand => subcommand.setName('recomendaciones')
        .setDescription('Muestra las recomendaciones con respecto al covid19'))
        .addSubcommand(subcommand => subcommand.setName('sintomas')
        .setDescription('Muestra los síntomas del covid19'))
        .addSubcommand(subcommand => subcommand.setName('covidpais')
        .setDescription('Muestra los datos de covid de cierto país')
        .addStringOption(option => option.setName('pais')
    ...
    run(client, interaction, options) {
        return __awaiter(this, void 0, void 0, function* () {
            if (interaction.options.getSubcommand() === 'casos') {
                axios.get('https://disease.sh/v3/covid-19/countries/argentina')
                    ...
                    const row = new MessageActionRow()
                        ...
                        .setLabel("😷 Total")
                        .setStyle("DANGER")).addComponents(new MessageButton()
                        .setCustomId("nextbtn")
                        .setLabel("📅 Hoy")
                        .setStyle("SUCCESS"));
            ...
            if (interaction.options.getSubcommand() === 'covidpais') {
                //Lista
                let pais = interaction.options.getString('pais');
                var pais2 = "";
                if (pais != null) {
                ...
                //Todos los paises
                if (pais2.length != 0) {
                    let paisEN = yield translate(pais2, { from: "es", to: "en" });
                    if (paisEN == "U.S")
                        paisEN = "USA";
                    let covidArg = yield axios.get(`https://disease.sh/v3/covid-19/countries/${paisEN}`);
                    const nombrePais = yield translate(covidArg.data.country, { from: "en", to: "es" });
                    const embed1 = new Discord.MessageEmbed()
