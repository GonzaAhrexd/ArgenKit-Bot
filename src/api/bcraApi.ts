...
export const getBaseMonetariaData = async (): Promise<BaseMonetariaData> => {
  try {
    ...
    const baseMonetariaValorAnterior =
      response.data.results[0].detalle[1].valor;

    const baseMonetariaData = {
      baseMonetariaValor: baseMonetariaValor,
      baseMonetariaValorAnterior: baseMonetariaValorAnterior,
      isAumentoBase:
        baseMonetariaValor > baseMonetariaValorAnterior
          ? "🔺"
          : "<:flechashaciaabajo:1210747546096369664>",
      fecha: response.data.results[0].detalle[0].fecha
        .split("-")
        .reverse()
        .join("/"),
    };
...
export const getInflacionData = async (): Promise<inflacionResponse> => {
  try {
    const startMonth = new Date(
      ...
      .toISOString()
      .split("T")[0];
    const startTwelveMonthsBefore = new Date(
      new Date().getFullYear(),
    ...
    const inflacionActualMes = new Date(
      inflacion.data.results[0].detalle[0].fecha,
    );
    const inflacionInteranualActualMes = new Date(
      interanual.data.results[0].detalle[0].fecha,
    );
    const inflacionInteranualAnteriorMes = new Date(
      interanual.data.results[0].detalle[0].fecha,
    );
