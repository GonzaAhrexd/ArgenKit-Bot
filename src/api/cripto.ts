...
export const getCriptoDataCoingecko = async (
  criptoId: string,
): Promise<CriptoDataCoingeckoResponse> => {
  try {
    const response = await coingeckoClient.get(
      `/${criptoId}/market_chart?vs_currency=usd&days=0`,
    );
    ...
    throw new Error(
      `Error al obtener datos de Coingecko para ${criptoId}: ${error}`,
    );
  }
};

export const getCriptoDataCriptoYa = async (
  criptoSymbol: string,
): Promise<CriptoDataYaResponse> => {
  try {
    const response = await criptoYaClient.get(`${criptoSymbol}`);
    return response.data;
  } catch (error) {
    throw new Error(
      `Error al obtener datos de CriptoYa para ${criptoSymbol}: ${error}`,
    );
  }
};

export const getAllCriptoData = async (
  criptoId: string,
  criptoSymbol: string,
) => {
  try {
    ...
    throw new Error(
      `Error al obtener datos para ${criptoId} y ${criptoSymbol}: ${error}`,
    );
  }
};
