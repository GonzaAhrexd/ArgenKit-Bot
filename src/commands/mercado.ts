    ...
    if (interaction.options.getSubcommand() === "estado") {
      ...
      try {
        ...
        const embed: EmbedBuilder = new EmbedBuilder()
          ...
          .setThumbnail(
            "https://cdn.discordapp.com/attachments/802944543510495292/1178904578763280456/stock.png?ex=6577d772&is=65656272&hm=d31896e4060b95d263afb323da2c5d36687da9f95998a024ab814dcfecb5d04b&",
          )
          .setDescription(
            `El mercado se encuentra  ${estadoMercado.isOpen ? "abierto" : "cerrado"}`,
          )
          .addFields(
            {
              name: "Feriado ",
              value: ` ${estadoMercado.holiday == null ? "Ninguno" : estadoMercado.holiday}`,
            },
            {
              name: "Estado",
              value: estadoMercado.isOpen ? "Abierto" : "Cerrado",
            },
            {
              name: "Sesión",
              value: `${estadoMercado.session == null ? "Ninguno" : estadoMercado.session}`,
            },
          );
        await wait(3000);
    ...
    if (interaction.options.getSubcommand() === "acciones") {
      ...
      try {
        ...
        const embedMantenimiento = new EmbedBuilder()
          ...
          .setColor("Yellow")
          .setDescription(
            "Este comando se encuentra en proceso de mantenimiento y optimización. Pronto estará disponible nuevamente. ¡Gracias por tu paciencia! De momento, se puede  seguir utilizando el subcomando `/mercado consultar` para consultar acciones específicas.",
          );
      ...
      try {
        ...
        const embed: EmbedBuilder = new EmbedBuilder()
          ...
          .addFields(
            {
              name: "Precio actual",
              value: `${formatoPrecio(activoValores.currentPrice, "USD")}\nARS${formatoPrecio(activoValores.currentPrice * dolarMep.venta, "ARS")}`,
              inline: true,
            },
            {
              name: "Precio anterior",
              value: `${formatoPrecio(activoValores.previousClosePrice, "USD")}\nARS${formatoPrecio(activoValores.previousClosePrice * dolarMep.venta, "ARS")}`,
              inline: true,
            },
            {
              name: "Variación anterior",
              value: `${formatoNum(activoValores.percentChange)}% ${subioPrecio(activoValores)}`,
              inline: true,
            },
            {
              name: "Precio de apertura",
              value: `${formatoPrecio(activoValores.openPriceOfDay, "USD")}\nARS${formatoPrecio(activoValores.openPriceOfDay * dolarMep.venta, "ARS")}`,
              inline: true,
            },
            {
              name: "Precio más bajo del día",
              value: `${formatoPrecio(activoValores.lowPriceOfDay, "USD")}\nARS${formatoPrecio(activoValores.lowPriceOfDay * dolarMep.venta, "ARS")}`,
              inline: true,
            },
            {
              name: "Precio más alto del día",
              value: `${formatoPrecio(activoValores.highPriceOfDay, "USD")}\nARS${formatoPrecio(activoValores.highPriceOfDay * dolarMep.venta, "ARS")}`,
              inline: true,
            },
          );
        await wait(3000);
