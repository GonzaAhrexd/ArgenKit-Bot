...
const crearEmbed = (conPercepciones: boolean) => {
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      // Mensual
      {
        name: "Suscripción mensual básico con anuncios",
        value: `ARS${formatoPrecio(calcular(6490), "ARS")}`,
        inline: true,
      },
      {
        name: "Suscripción mensual estándar",
        value: `ARS${formatoPrecio(calcular(8390), "ARS")}`,
        inline: true,
      },
      {
        name: "Suscripción mensual platino",
        value: `ARS${formatoPrecio(calcular(10090), "ARS")}`,
        inline: true,
      },
      // Anual
      {
        name: "Suscripción anual básico con anuncios",
        value: `ARS${formatoPrecio(calcular(56590), "ARS")}`,
        inline: true,
      },
      {
        name: "Suscripción anual estándar",
        value: `ARS${formatoPrecio(calcular(70590), "ARS")}`,
        inline: true,
      },
      {
        name: "Suscripción anual platino",
        value: `ARS${formatoPrecio(calcular(84090), "ARS")}`,
        inline: true,
      },
    );
};
